...
from .serializers import (
    BaseAnalysisSerializer,
    GridTileAnalysisSerializer,
    SurveyAnalysisSerializer,
)
...
class GridTileAnalysisViewSet(BaseListSerializerMixin, viewsets.ReadOnlyModelViewSet):
    ...
    queryset = (
        GridTile.objects.prefetch_related('hours')
        .exclude(hours__isnull=True)
        .annotate(
            with_kea=Count(
                Case(When(hours__kea=True, then=1), output_field=IntegerField())
            ),
            total=Count('hours'),
        )
    )
...
class SurveyAnalysisViewSet(BaseListSerializerMixin, viewsets.ReadOnlyModelViewSet):
    ...
    queryset = Survey.objects.prefetch_related('hours').annotate(
        with_kea=Count(
            Case(When(hours__kea=True, then=1), output_field=IntegerField())
        ),
        total=Count('hours'),
